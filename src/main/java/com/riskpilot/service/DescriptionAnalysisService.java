...
/**
 * Service for analyzing job/internship descriptions.
 * 
 * Analysis Pipeline:
 * 1. Parse description to extract emails, URLs, and company name
 * 2. Fetch online reviews for the detected company
 * 3. Perform URL verification if URLs are found
 * 4. Send to AI for comprehensive scam detection analysis (with review context)
 * 5. Return combined analysis response with risk assessment and reviews
 * 
 * This follows the same logic as file analysis but works with raw text input,
 * and enhances analysis with online company reputation data.
 */
@Service
public class DescriptionAnalysisService {
    ...
    /**
     * Main method that orchestrates the full AI-powered description analysis pipeline:
     * 1. Parse description text to extract emails, URLs, and company name
     * 2. Fetch online reviews for the detected company
     * 3. Perform URL verification if URLs are found
     * 4. Send to AI for scam detection analysis (with review context)
     * 5. Return combined analysis response
     */
    public AiAnalysisResponseDto analyzeDescription(String jobDescription) {
        // Step 1: Validate input
        if (jobDescription == null || jobDescription.isBlank()) {
            throw new RuntimeException("Job description text is required and cannot be empty.");
        }
        
        System.out.println("📝 Analyzing description: " + jobDescription.substring(0, Math.min(100, jobDescription.length())) + "...");

        // Step 2: Parse content to identify structured fields
        String email = contentParserService.extractAllEmails(jobDescription)
            .stream()
            .findFirst()
            .orElse(null);
        
        String url = contentParserService.extractAllUrls(jobDescription)
            .stream()
            .findFirst()
            .orElse(null);
        ...
        aiRequest.setExtractedText(jobDescription);
        ...
        System.out.println("🤖 Sending to AI analysis with review context...");
        ...
        if (companyName == null || companyName.isBlank()) {
            ...
            // PRIORITY 4: Content parsing (last resort)
            if (companyName == null || companyName.isBlank()) {
                String companyFromContent = contentParserService.extractCompanyName(jobDescription);
     ...
     * Example: careers.amazon.com -> Amazon
     */
    private String extractCompanyFromUrl(String url) {
        ...
        try {
            ...
            if (parts.length > 0) {
                ...
                // Skip if it's a subdomain like "careers", "jobs", "apply"
    ...
    private void enhanceResponseWithUrlAnalysis(AiAnalysisResponseDto aiResponse, UrlAnalysisResponse urlAnalysis) {
        // If URL analysis shows HIGH risk, add it to the indicators and lower trust score
        if ("HIGH".equals(urlAnalysis.getRiskLevel())) {
            ...
            // Lower trust score based on URL risk
            double urlTrustScore = urlAnalysis.getTrustScore();
            double currentTrustScore = aiResponse.getTrustScore();
            
            // Average the trust scores but weight URL analysis if it's particularly risky
            if (urlTrustScore < 3.0) {
                // URL is very risky, weight it heavily
        ...
        // Add URL-specific indicators to suggestions
